...
function ρz_test(ρx, dA::UnivariateDistribution, dB::UnivariateDistribution)
    ρz_val = @match (dA, dB) begin
        ...
        (Uniform(0, 1), Binomial(1, 0.5)) || (Binomial(1, 0.5), Uniform(0, 1)) =>
            √(2) * sin(ρx * π / (2*√(3)))

        (Uniform(0, 1), Normal(0, 1)) || (Normal(0, 1), Uniform(0, 1)) =>
            ρx * √(π / 3)

        (Binomial(1, 0.5), Binomial(1, 0.5)) =>
            sin(ρx * π / 2)

        (Binomial(1, 0.5), Normal(0, 1)) || (Normal(0, 1), Binomial(1, 0.5)) =>
            ρx * √(π / 2)

        (Normal(0, 1), LogNormal(0, σ)) =>
            ρx * √(exp(σ^2)-1) / σ
        (LogNormal(0, σ), Normal(0, 1)) =>
            ρx * √(exp(σ^2)-1) / σ

        (LogNormal(0, σ₁), LogNormal(0, σ₂)) =>
            (ρx * √((exp(σ₁^2) - 1)*(exp(σ₂^2) - 1)) + 1) / exp(σ₁ * σ₂)

        (Uniform(0, 1), LogNormal(0, σ)) =>
            (√(2) / σ) * quantile(Normal(0, 1), ρx * √(exp(σ^2) - 1) / (2*√(3)) + 0.5)
        (LogNormal(0, σ), Uniform(0, 1)) =>
            (√(2) / σ) * quantile(Normal(0, 1), ρx * √(exp(σ^2) - 1) / (2*√(3)) + 0.5)

        (Binomial(1, 0.5), LogNormal(0, σ)) =>
            quantile(Normal(0, 1), 0.5 * ρx * √(exp(σ^2) - 1) + 0.5) / σ
        (LogNormal(0, σ), Binomial(1, 0.5)) =>
            quantile(Normal(0, 1), 0.5 * ρx * √(exp(σ^2) - 1) + 0.5) / σ
